...
function fromReader(markdown:string,url:string){
 ...
 return {title:clean(title),images,price:num(priceMatch?.[1]),rating:num(ratingMatch?.[1]),reviewCount:Number(String(reviewMatch?.[1]||"0").replace(/,/g,""))||0,description:clean(markdown.replace(/^#.*$/m,"").split("\n\n").find(x=>x.trim())||""),url};
}
