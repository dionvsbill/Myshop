...
function scripts(html:string){const out:any[]=[];const re=/<script[^>]*type=["']application\\/ld\\+json["'][^>]*>([\\s\\S]*?)<\\/script>/gi;let m:RegExpExecArray|null;while((m=re.exec(html))!==null){try{out.push(JSON.parse(m[1].trim()))}catch{}}return out}
