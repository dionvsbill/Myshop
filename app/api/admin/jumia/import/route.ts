...
function extractImages(source:string){const out:string[]=[];const re=/https?:\/\/[^\s"'<>\\)]+/gi;let m:RegExpExecArray|null;while((m=re.exec(source))!==null){const x=m[0].replace(/\\/g,"").replace(/&amp;/g,"&");if(/(?:jpg|jpeg|png|webp)/i.test(x)&&/jumia/i.test(x)&&!/(?:favicon|logo|sprite|icon)/i.test(x)&&out.indexOf(x)===-1)out.push(x);if(out.length>=12)break}return out}
