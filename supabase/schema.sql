...
create index if not exists products_category_idx on products(category_id); create index if not exists products_active_idx on products(is_active); create index if not exists orders_user_idx on orders(user_id); create index if not exists cart_user_idx on cart_items(user_id); create index if not exists cart_product_idx on cart_items(product_id); create index if not exists cart_variant_idx on cart_items(variant_id); create index if not exists reviews_product_idx on reviews(product_id); create index if not exists reviews_user_idx on reviews(user_id);
...
drop policy if exists product_options_admin_write on \1;
create policy product_options_admin_insert on \1 for insert to authenticated with check(exists(select 1 from profiles pr where pr.id=(select auth.uid()) and pr.role='ADMIN'));
create policy product_options_admin_update on \1 for update to authenticated using(exists(select 1 from profiles pr where pr.id=(select auth.uid()) and pr.role='ADMIN')) with check(exists(select 1 from profiles pr where pr.id=(select auth.uid()) and pr.role='ADMIN'));
create policy product_options_admin_delete on \1 for delete to authenticated using(exists(select 1 from profiles pr where pr.id=(select auth.uid()) and pr.role='ADMIN'));
...
drop policy if exists product_option_values_admin_write on \1;
create policy product_option_values_admin_insert on \1 for insert to authenticated with check(exists(select 1 from profiles pr where pr.id=(select auth.uid()) and pr.role='ADMIN'));
create policy product_option_values_admin_update on \1 for update to authenticated using(exists(select 1 from profiles pr where pr.id=(select auth.uid()) and pr.role='ADMIN')) with check(exists(select 1 from profiles pr where pr.id=(select auth.uid()) and pr.role='ADMIN'));
create policy product_option_values_admin_delete on \1 for delete to authenticated using(exists(select 1 from profiles pr where pr.id=(select auth.uid()) and pr.role='ADMIN'));
...
drop policy if exists product_variants_admin_write on \1;
create policy product_variants_admin_insert on \1 for insert to authenticated with check(exists(select 1 from profiles pr where pr.id=(select auth.uid()) and pr.role='ADMIN'));
create policy product_variants_admin_update on \1 for update to authenticated using(exists(select 1 from profiles pr where pr.id=(select auth.uid()) and pr.role='ADMIN')) with check(exists(select 1 from profiles pr where pr.id=(select auth.uid()) and pr.role='ADMIN'));
create policy product_variants_admin_delete on \1 for delete to authenticated using(exists(select 1 from profiles pr where pr.id=(select auth.uid()) and pr.role='ADMIN'));
...
drop policy if exists product_media_admin_write on \1;
create policy product_media_admin_insert on \1 for insert to authenticated with check(exists(select 1 from profiles pr where pr.id=(select auth.uid()) and pr.role='ADMIN'));
create policy product_media_admin_update on \1 for update to authenticated using(exists(select 1 from profiles pr where pr.id=(select auth.uid()) and pr.role='ADMIN')) with check(exists(select 1 from profiles pr where pr.id=(select auth.uid()) and pr.role='ADMIN'));
create policy product_media_admin_delete on \1 for delete to authenticated using(exists(select 1 from profiles pr where pr.id=(select auth.uid()) and pr.role='ADMIN'));
