  ...
  useEffect(()=>{
    ...
    (async()=>{
      ...
      const [{data:userResult},{data:order}] = await Promise.all([
        ...
        supabase.from("orders").select("id,order_number,status,total,currency,payment_reference,created_at,shipping_address,order_items(id,title,variant_title,quantity,unit_price,image)").eq("id",orderId).maybeSingle()
      ]);

      if(!userResult.user||!order)return;

      const items=(order.order_items||[]).map((item:any)=>
        `${item.title}${item.variant_title?` — ${item.variant_title}`:""} × ${item.quantity} @ ${order.currency||"GHS"} ${Number(item.unit_price||0).toFixed(2)}`
      ).join("\n");
