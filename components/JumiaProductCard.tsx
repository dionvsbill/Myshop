...
export default function JumiaProductCard({product}:{product:JumiaCardItem}){
 ...
 return <article className={styles.card}>
  <a href={product.source_url} target="_blank" rel="noreferrer" className={styles.imageLink}>
   <div className={styles.imageWrap}><img src={product.images?.[0]?"/api/jumia/image?url="+encodeURIComponent(product.images[0]):"https://www.jumia.com.gh/favicon.ico"} alt={product.title} loading="lazy"/>{discount>0&&<span className={styles.badge}>-{discount}%</span>}</div>
  </a>
